...
func (m *Model) handleEventAddKeys(msg tea.KeyMsg) bool {
	if !m.EventAddMenu || m.ActiveSheet != types.SheetCalendar {
		return false
	}
	key := msg.String()
	switch key {
...
func sortSchedule(s []types.ScheduleEntry) {
	...
	startMins := func(e types.ScheduleEntry) int {
		var h, m int
